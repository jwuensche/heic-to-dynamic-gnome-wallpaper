...
pub fn get_wallpaper_metadata(image_ctx: &HeifContext) -> Option<WallPaperMode> {
    ...
    image_ctx.primary_image_handle().unwrap().metadata_block_ids("mime", &mut metadatas);
        ...
        loop {
            match reader.read_event(&mut buf) {
                Ok(quick_xml::events::Event::Empty(ref e)) => {
                    e.attributes()
                        .filter(|att| att.as_ref().unwrap().key == "apple_desktop:h24".as_bytes() || att.as_ref().unwrap().key == "apple_desktop:solar".as_bytes())
                        .for_each(|att| {

                            match att.as_ref().unwrap().key {
                                s if s == "apple_desktop:h24".as_bytes() => h24 = Some(WallPaperMode::H24(String::from_utf8_lossy(&att.unwrap().value.into_owned()).to_string())),
                                s if s == "apple_desktop:solar".as_bytes() => h24 = Some(WallPaperMode::Solar(String::from_utf8_lossy(&att.unwrap().value.into_owned()).to_string())),
